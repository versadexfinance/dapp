...
import { Token, Tokens } from 'web3/types';
...
export function useSwap({
    ...
    const contract = new ethers.Contract(tokenIn?.address, erc20ABI , provider);
    ...
    const swap = useCallback(async () => {
        try {
            ...
            let balanceTokenIn = null;

           if(tokenIn?.ticker !== "WETH"){
            const balance = await  contract.balanceOf(address) 
            balanceTokenIn = ethers.BigNumber.from(balance) // Wei
